...
TEST_SOURCES := $(TESTS:%.test=test_%.cpp)
...
DEPFILES := $(TARGET:.hex=.d) $(CLASS_SOURCES:.cpp=.d) $(TEST_SOURCES:.cpp=.d)
$(DEPFILES):
...
.SECONDARY : $(TARGET:.hex=.avr_o) $(TARGET:.hex=.elf) $(TARGET) $(CLASS_SOURCES:.cpp=.avr_o) $(TEST_SOURCES:.cpp=.avr_o)
